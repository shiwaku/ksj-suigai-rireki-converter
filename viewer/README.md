...
| 地形（Mapterhorn） | **段彩**（標高レンジ選択・凡例つき）/ 陰影起伏（5方式・強調可変）/ 3D地形（起伏倍率可変）/ 等高線。段彩＋陰影起伏で陰影段彩図になる |
...
そこでレンジを選べるようにし、指定レンジでは色を等間隔に引き伸ばす。

| レンジ | 1段あたり |
|---|---|
| 全国（絶対標高 -10〜4000m） | 帯ごとに 1m〜1000m |
| 山地 0〜1000m | 71.4m |
| 平野 0〜100m | 7.14m |
| 低地 0〜20m（既定） | 1.43m |
| 微地形 0〜5m（窪地） | 0.357m |

同じ 34cm の標高差に対する色の差は、全国レンジで 4/255、微地形レンジで 68/255（17倍）。
`check:relief` がこの比を検証している。

引き伸ばすときは**先頭の色を1つ落として15色**にする。`TINTS` の -10m と 0m は
意図的に同色（海面下と海面を同じ色で塗る）で、そのまま等間隔に割ると最下段の2段が
同色になり、レンジの下端——低地でいちばん見たい足元——が1段ぶん潰れてしまう。

レンジは**タイルURLに埋め込む**（`relief://<mode>/<min>/<max>/<DEM URL>`）。
...
指定レンジでは色を等間隔に割っているので、等幅がそのまま実際の間隔になる。
